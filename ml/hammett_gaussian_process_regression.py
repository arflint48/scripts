######
#gaussian process regression on hammett nn data
#created may 25, 2020
...
#Normalization
norm_out = np.append(normalize(output,axis=0),np.reshape(ids,(ids.shape[0],1)),1)
...
def GaussianProcessRegressor(inp_train, out_train, inp_test, out_test, ids, l):
	global gprtestInd, gprtestAvg
	...
	gprmod = gaussian_process.GaussianProcessRegressor(kernel= kernel)
	gprmod.fit(trainX,trainY)
	gprtrainrsquared = gprmod.score(trainX, trainY)
	gprrsquared=gprmod.score(FullInp,norm_out[:,l])
	if ((gprmod.score(FullInp,norm_out[:,l]) < (0.9*gprtrainrsquared)) or (gprmod.score(FullInp,norm_out[:,l]) > (1.1*gprtrainrsquared))):
		n=1
		return n
	
	#predicts energies
	gprpred = gprmod.predict(FullInp)
	gprpred=np.reshape(gprpred,(gprpred.shape[0],1))
	#add the molecule names
	#gprpred2 = np.concatenate([mols, gprpred.reshape(mollength,1)], axis = 1)
	#the individual runs have the molecule names
	if gprtestInd.shape[0]==0:
		gprtestInd=np.array([gprrsquared])
	else:
		gprtestInd=np.append(gprtestInd,np.array([gprrsquared]),0)

	if gprtestAvg.shape[0]==0:
		gprtestAvg=gprpred
	else:
		gprtestAvg=np.append(gprtestAvg,gprpred,1)
	return n
...
for q in range(0,35):
	labstrt=0+q
	labend=1+q
	norm_out = np.append(normalize(output[:,labstrt:labend],axis=0),np.reshape(ids,(ids.shape[0],1)),1)
	for l in range(0,(labend-labstrt)):# used to select output value
		for k in range(17,18): #used for test size selection
			for j in range(0,350): #How many linear regressions to do.
				inp_train, inp_test, out_train, out_test = train_test_split(FullInp, norm_out, test_size=(0.05*k))
				GaussianProcessRegressor(inp_train, out_train, inp_test, out_test, ids, l)

			if gprtestAvg.shape[0] != 0:
			#Get average for prediction 
				avg = np.mean(gprtestAvg,1)
				rsquaredavg = np.mean(gprtestInd,0)
...
#GaussianProcessRegressor(inp_train, out_train, inp_test, out_test, ids)
